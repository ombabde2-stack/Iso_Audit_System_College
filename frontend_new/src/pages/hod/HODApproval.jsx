...
import { CheckCircle, XCircle, RotateCcw, ArrowLeft, Download, ExternalLink, TableProperties, CheckCircle2, BadgeCheck } from 'lucide-react';
import toast from 'react-hot-toast';
import { getSingleForm, getFormDownloadUrl } from '../../api/forms.api';
import { updateFormStatus } from '../../api/review.api';
...
export default function HODApproval() {
  ...
  const handleAction = async () => {
    if ((modal === 'REJECTED' || modal === 'RETURNED') && !remarks.trim()) {
      toast.error('Remarks are required for rejection/return');
      return;
    }
    setSubmitting(true);
    try {
      await updateFormStatus(id, { status: modal, remarks });
      toast.success(`Form ${modal.toLowerCase()} successfully!`);
      navigate('/hod-queue');
    } catch (err) {
  ...
  if (loading) return <div className="max-w-3xl mx-auto space-y-4">{Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-12" />)}</div>;
  ...
  return (
    <div className="max-w-3xl mx-auto animate-slide-up space-y-4">
      ...
      <div className="card p-6 space-y-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 className="text-xl font-bold mt-1">{form.formTitle || 'Form Review'}</h2>
            <p className="text-sm text-slate-500">Submitted by {form.submittedBy?.name} · {formatDateTime(form.createdAt)}</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm border-t border-slate-100 dark:border-slate-800 pt-4">
          <div className="flex items-center gap-2 text-slate-500">
            ...
            <span>Status Updated: {formatDateTime(form.updatedAt)}</span>
          </div>
          <div className="flex items-center gap-2 text-slate-500">
            <RotateCcw size={14} /> 
        ...
        {/* Smart Data Section (Shared Intelligence) */}
        ...
        <div className="rounded-xl border border-slate-200 dark:border-slate-800 p-4 space-y-4">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Submitted Document</p>
          {submittedFileUrl ? (
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="font-medium text-slate-800 dark:text-slate-100">{submittedFile?.filename}</p>
                <p className="text-sm text-slate-500 mt-1">Review the exact uploaded Word/Excel file before approving, returning, or rejecting.</p>
              </div>
              <div className="flex gap-2">
                ...
                <a
                  href={getFormDownloadUrl(form._id)}
                  download={submittedFile?.filename}
            ...
            <p className="text-sm text-slate-400">No uploaded file found for this submission.</p>
          )}
        </div>

        {form.submitterRemarks && (
          <div className="rounded-xl bg-amber-50 dark:bg-amber-950/20 p-4">
            <p className="text-xs font-semibold text-amber-700 dark:text-amber-400 mb-1 uppercase tracking-wide">Submission Notes</p>
        ...
        {form.status === 'PENDING' && (
          <div className="flex gap-3 pt-2 border-t border-slate-100 dark:border-slate-800">
            <Button variant="success" icon={CheckCircle} onClick={() => { setModal('APPROVED'); setRemarks(''); }} className="flex-1">
              Approve
            </Button>
            <Button variant="outline" icon={RotateCcw} onClick={() => { setModal('RETURNED'); setRemarks(''); }} className="flex-1">
              Return
            </Button>
            <Button variant="danger" icon={XCircle} onClick={() => { setModal('REJECTED'); setRemarks(''); }} className="flex-1">
              Reject
            </Button>
          </div>
        )}
        {form.status !== 'PENDING' && (
          <div className="text-center py-3 text-sm text-slate-400">This form has already been {form.status.toLowerCase()}.</div>
        )}
      </div>
      ...
      <Modal
        ...
        title={`${modal === 'APPROVED' ? 'Approve' : modal === 'REJECTED' ? 'Reject' : 'Return'} Form`}
        footer={
          <>
            <Button variant="outline" onClick={() => setModal(null)}>Cancel</Button>
            <Button
              variant={modal === 'APPROVED' ? 'success' : modal === 'REJECTED' ? 'danger' : 'primary'}
              loading={submitting}
              onClick={handleAction}
            >
              Confirm {modal?.charAt(0) + modal?.slice(1).toLowerCase()}
            </Button>
          </>
        }
      >
        <div className="space-y-1.5">
          <label className="text-sm font-medium text-slate-700 dark:text-slate-300">
            Remarks {modal !== 'APPROVED' && <span className="text-rose-500">*</span>}
          </label>
          <textarea
            value={remarks}
            onChange={(e) => setRemarks(e.target.value)}
            className="input-base min-h-[90px] resize-none"
            placeholder={modal === 'APPROVED' ? 'Add optional comments...' : 'Provide feedback or reason...'}
          />
        </div>
      </Modal>
    </div>
